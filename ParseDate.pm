...
use vars
	qw($VERSION %SSECONDS %NMONTHS $MONTHRX $DATERX $TIMERX %MATCHES $RX00);
$VERSION = '1.003';
...
%MATCHES = (
	...
	'50' => [    # 22.12.2004 12:13:14
		sub {
			...
			timelocal($sec, $min, $hour, $data->[0], $data->[1] - 1,
				$data->[2]);
		}
	],
	...
	'55' => [    # 2004.12.22 12:13:14
		sub {
			my ($now, $data) = @_;
			my ($hour, $min, $sec) = split(/:/, $data->[3]);
			timelocal($sec, $min, $hour, $data->[1], $data->[1] - 1,
				$data->[0]);
		}
	],
	...
	'65' => [    # 2004-12-22 12:13:14
		sub {
			my ($now, $data) = @_;
			my ($hour, $min, $sec) = split(/:/, $data->[3]);
			timelocal($sec, $min, $hour, $data->[2], $data->[1] - 1,
				$data->[0]);
		}
	],
	...
	'70' => [    # 12:13:14 22.12.2004
		sub {
			my ($now, $data) = @_;
			my ($hour, $min, $sec) = split(/:/, $data->[0]);
			timelocal($sec, $min, $hour, $data->[1], $data->[2] - 1,
				$data->[3]);
		}
	],
	...
	'90' => [    # 12 Apr 2004 12:13:14
		sub {
			my ($now, $data) = @_;
			my ($hour, $min, $sec) = split(/:/, $data->[3]);
			timelocal($sec, $min, $hour, $data->[0], _getMonth($data->[1]),
				$data->[2]);
		}
	],
	...
	'100' => [    # Apr 12 2004 12:13:14
		sub {
			my ($now, $data) = @_;
			my ($hour, $min, $sec) = split(/:/, $data->[3]);
			timelocal($sec, $min, $hour, $data->[1], _getMonth($data->[0]),
				$data->[2]);
		}
	],
	...
	'120' => [    # 12 Mar 2004
		sub {
			my ($now, $data) = @_;
			timelocal(3, 2, 1, $data->[0], _getMonth($data->[1]), $data->[2]);
		}
	],
	...
	'130' => [    # 12.08.2004
		sub {
			my ($now, $data) = @_;
			timelocal(3, 2, 1, $data->[0], $data->[1] - 1, $data->[2]);
		}
	],
	...
	'140' => [    # 2004.08.22
		sub {
			my ($now, $data) = @_;
			timelocal(3, 2, 1, $data->[2], $data->[1] - 1, $data->[0]);
		}
	],
